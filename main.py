...
def App():
	try:
		if not os.path.exists(users_bin_path) or os.path.getsize(users_bin_path) == 0:
			...
			input("Press enter to proceed to login...")
		...
		try:
			...
			while True:
				try:
					...
					if user.check_role(username).lower() == "admin":
						if choice in ["A1", "a1"]:
							target_username = str(input("Enter username: "))
							user.add_users(target_username)
							print("-" * 80)
							...
							username_to_edit = str(input("Enter the username of the user to edit: "))
							user.edit_user(username_to_edit)
							print("-" * 80)
							input("press any key to continue")
							os.system("cls")
							continue
							...
							print("-" * 80)
							target_username = str(input("Enter username: "))
							user.delete_user(target_username)
							input("press any key to continue")
							os.system("cls")
							continue
